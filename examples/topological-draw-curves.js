...
/** Snap-only source of currently allowed control-point targets. */
const controlPointSnapSource = new VectorSource();
...
/**
 * The single flat-polyline crossing predicate for the example. Calls `callback`
 * with each proper-interior crossing (0 < t,u < 1 and farther than
 * CROSSING_EPSILON_SQ from every endpoint) and stops early on a truthy return.
 * The epsilon filter and the parallel/collinear skip keep intentionally-shared
 * trace-hugged boundaries (endpoint touches, collinear overlaps) from reading as
 * crossings.
...
function checkOverlapWithExisting(
  ...
  for (const feat of features) {
    ...
    // Prefer the arc-aware primitive when both sides expose curve segments: its
    // reversal-aware same-arc skip treats a trace-hugged boundary as shared, not
    // crossing (tessellation would read spurious crossings at the trace exit).
    // Fall back to tessellation only for non-curve sources.
...
/**
 * Commit-time tripwire: every coordinate in a canonicalized trace run must be an
 * exact source control point. Only traced index ranges are checked (free /
 * edge-snapped coordinates legitimately lie on a boundary); the match is exact
 * modulo float drift, so a leaked tessellated arc sample is caught.
...
/**
 * Validate the ENTIRE scene for self-intersections and pair-wise crossings,
 * rendering a red dot at each. Must be global: setTopoStatus rebuilds the
 * overlay from scratch, so validating only the edited feature would erase the
 * markers for every other crossing pair. Updates `validationState`.
...
function validateFeatures() {
  ...
  /** @type {Array<Array<number>>} */
  const allCrossings = [];
  // `Map` here is ol/Map.js (imported), whose set() stringifies object keys —
  // use native `window.Map` so per-feature arc lookups stay distinct.
  ...
  for (const f of allFeatures) {
    ...
    // Self-check every curve type, not just CurvePolygon: open curves loop too.
    if (arcs.length > 0) {
      const selfX = getArcArraySelfIntersections(
        arcs,
        CROSSING_EPSILON_SQ,
        SAME_ARC_TOLERANCE_SQ,
      );
  ...
  for (let i = 0, ii = allFeatures.length; i < ii; i++) {
    ...
    for (let j = i + 1; j < ii; j++) {
      ...
      if (arcsA.length > 0 && arcsB.length > 0) {
        ...
        // Iterate each ring separately: a single 0..lastEnd span would stitch
        // one ring's last vertex to the next ring's first, inventing a phantom
        // segment that registers bogus crossings.
        let aStart = 0;
 ...
 * @return {string} Stable key for exact shared control points.
 */
function getControlPointKey(coordinate) {
  return coordinate[0] + ',' + coordinate[1];
}

/**
 * @param {import('../src/ol/Feature.js').default} feature Feature.
 * @return {Set<string>} Exact control-point coordinate keys.
 */
function getFeatureControlPointKeys(feature) {
  const keys = new window.Set();
  const geom = feature.getGeometry();
  if (!geom) {
    return keys;
  }
  for (const point of getControlPoints(geom)) {
    keys.add(getControlPointKey(point.coord));
  }
  return keys;
}

/**
 * Find only the shared control-point coordinates between seed features and
 * their neighbors. A connected neighbor remains a valid snap target at its
 * non-shared points; only the actual shared node is self for this gesture.
 * @param {Iterable<import('../src/ol/Feature.js').default>} seedFeatures Seeds.
 * @return {Set<string>} Shared control-point coordinate keys.
 */
function getSharedControlPointKeys(seedFeatures) {
  const seedSet = new window.Set(seedFeatures);
  const seedKeys = new window.Set();
  for (const feature of seedSet) {
    for (const key of getFeatureControlPointKeys(feature)) {
      seedKeys.add(key);
    }
  }

  const sharedKeys = new window.Set();
  for (const feature of source.getFeatures()) {
    if (feature.get('_snapPoint') || seedSet.has(feature)) {
      continue;
    }
    for (const key of getFeatureControlPointKeys(feature)) {
      if (seedKeys.has(key)) {
...
/**
 * Original control-point coordinates on the edited feature that remain snap
 * targets during the current modify gesture, so a dragged point can snap back.
 * @type {Set<string>}
 */
const snapAllowedOriginalPointKeys = new window.Set();

/**
 * Rebuild the visible control point overlay and the snap-only candidate source.
 * All control points are always rendered visually. Edited-feature control
 * points are excluded from the snap candidate source, except for the grabbed
 * point's original coordinate so the user can snap back to it.
 * @param {Set<import('../src/ol/Feature.js').default>} [excludedFeatures]
 * Features whose control points should not be snappable, except allowed originals.
 * @param {Set<string>} [excludedPointKeys] Exact shared control points not to snap to.
 */
function rebuildControlPointSource(excludedFeatures, excludedPointKeys) {
  controlPointSource.clear();
  controlPointSnapSource.clear();
  for (const feat of source.getFeatures()) {
    ...
    for (const pt of getControlPoints(geom)) {
      const pointKey = getControlPointKey(pt.coord);
      const excludedFeature = excludedFeatures && excludedFeatures.has(feat);
      const allowedOriginal =
        excludedFeature && snapAllowedOriginalPointKeys.has(pointKey);
      const excludedPoint =
        excludedPointKeys && excludedPointKeys.has(pointKey);
      ...
      if (!allowedOriginal && (excludedFeature || excludedPoint)) {
        continue;
      }
      controlPointSnapSource.addFeature(
        new Feature({
          geometry: new Point(pt.coord),
          _controlPoint: true,
          _sourceFeature: feat,
...
// Dedicated vertex snap on exact control-point candidates. Generous tolerance
// so the user can reliably grab a vertex to start tracing.
const controlPointSnap = new Snap({
  source: controlPointSnapSource,
  vertex: true,
  edge: false,
  pixelTolerance: 20,
});
...
/**
 * Features currently removed from the source `snap` because they are (or are
 * about to be) modified. A vertex being dragged must NEVER snap onto its own
 * feature, so the feature is pulled out of the snap index for the duration of
 * the gesture; entries are restored after modifyend.
 * @type {Set<import('../src/ol/Feature.js').default>}
 */
const snapExcludedFeatures = new window.Set();

/**
 * Exact shared control points that should not be snap targets during the
 * current modify gesture, even when they belong to neighboring features.
 * @type {Set<string>}
 */
const snapExcludedPointKeys = new window.Set();

/**
 * Remove a feature from the source `snap` for the duration of a modify gesture,
 * tracking it so it is restored exactly once. removeFeature also drops the
 * per-feature change listener, so the constant geometry updates during the drag
 * cannot silently re-index it.
 * @param {import('../src/ol/Feature.js').default} feature Feature to exclude.
 */
function excludeFromSnap(feature) {
  if (feature.get('_snapPoint') || snapExcludedFeatures.has(feature)) {
    return;
  }
  snapExcludedFeatures.add(feature);
  snap.removeFeature(feature);
  rebuildControlPointSource(snapExcludedFeatures, snapExcludedPointKeys);
}

/**
 * Exclude the edited features and only the exact shared points on neighbors.
 * @param {Iterable<import('../src/ol/Feature.js').default>} features Seeds.
 */
function excludeSelfAndSharedPointsFromSnap(features) {
  for (const key of getSharedControlPointKeys(features)) {
    snapExcludedPointKeys.add(key);
  }
  for (const feature of features) {
    excludeFromSnap(feature);
  }
  rebuildControlPointSource(snapExcludedFeatures, snapExcludedPointKeys);
}

/** Restore every feature excluded during the current modify gesture. */
function restoreSnapExclusions() {
  if (!snapExcludedFeatures.size && !snapExcludedPointKeys.size) {
    return;
  }
  for (const feature of snapExcludedFeatures) {
    snap.addFeature(feature);
  }
  snapExcludedFeatures.clear();
  snapExcludedPointKeys.clear();
  snapAllowedOriginalPointKeys.clear();
  rebuildControlPointSource();
}

const rawSnapTo = snap.snapTo.bind(snap);
snap.snapTo = function (pixel, pixelCoordinate, map) {
  const result = rawSnapTo(pixel, pixelCoordinate, map);
  if (result && snapExcludedPointKeys.has(getControlPointKey(result.vertex))) {
    return null;
  }
  return result;
};

// On pointerdown (before Snap, and before Modify's first-drag `modifystart`)
// exclude the grabbed feature so it can't self-snap. Identify it via
// snap.snapTo (vertex/edge-based, like Modify); getClosestFeatureToCoordinate
// is fill-aware and would return an overlapping polygon instead of the line.
map.on('pointerdown', (event) => {
  if (!modify.getActive()) {
    return;
  }
  const result = snap.snapTo(event.pixel, event.coordinate, event.map);
  if (result && result.feature) {
    snapAllowedOriginalPointKeys.add(getControlPointKey(result.vertex));
    excludeSelfAndSharedPointsFromSnap([result.feature]);
  }
});

// A click near a vertex can exclude a feature on pointerdown without ever
// starting a modify drag. Restore that no-drag case on pointerup. Real drags
// restore in modifyend, after Snap has processed the final pointerup.
map.on('pointerup', () => {
  if (!modifyActive) {
    restoreSnapExclusions();
  }
});
...
modify.on('modifystart', (event) => {
  ...
  // A vertex being modified must NEVER snap onto its own feature. If another
  // feature shares a control point with it, suppress only that shared point;
  // the neighbor's non-shared points remain valid snap targets.
  excludeSelfAndSharedPointsFromSnap(modifyTargetFeatures);
});
...
modify.on('modifyend', () => {
  ...
  // Restore after Snap has processed the final pointerup, so self targets are
  // never reintroduced during the drag gesture.
  restoreSnapExclusions();
...
function wireTraceLifecycle(drawInteraction) {
  ...
  drawInteraction.on('tracestart', () => {
    traceActive = true;
    currentTraceFeature = null;
    // tracestart fires before the entry click commits its vertex, which lands
    // at the current tip slot (length - 1).
  ...
  drawInteraction.on('trace', (e) => {
    // Accumulate every feature this trace touches (it may junction-hop A→B→…)
    // from the continuous event so all hugged features stay excluded from
    // containment/crossing checks.
    const trace = e.trace;
    if (trace && trace.feature) {
      currentTraceFeature = trace.feature;
      traceExcludedFeatures.add(trace.feature);
    }
    if (
      trace &&
      trace.geometry &&
      !traceBoundaryRings.includes(trace.geometry)
    ) {
      traceBoundaryRings.push(trace.geometry);
    }
    // Live preview: render the traced run as one LineString by keeping exactly
    // one 'line' break at the entry vertex (find-or-replace, not append, so
    // backtracking can't strand a break at a vanished index). Forcing 'line'
    // matters when the trace is the first draw action (entryIdx 0 was seeded
    // 'arc'); otherwise buildSketchSubs fits a rogue CircularString through the
  ...
  drawInteraction.on('traceend', (e) => {
    ...
    // Sketch tail is now canonical (canonicalizeTraceRun_ ran). Stamp segment
    // breaks at the canonical indices from trace.canonicalEdges, walking
    // control-point counts from the entry.
    const edges = (trace && trace.canonicalEdges) || [];
    ...
    // Rebuild the sketch geometry from the canonical coordinates + fresh breaks
    // and revalidate now: nothing else reruns geometryFunction until the next
    // pointermove, so a finish/auto-close in that window would otherwise read a
...
function addDrawInteraction() {
  ...
  if (mode === 'None') {
    ...
    snap.setActive(true);
    // Control-point snapping stays on in edit mode so a moved vertex can snap to
    // another feature's exact control points.
    controlPointSnap.setActive(true);
    sketchSnap.setActive(false);
    status(
  ...
  draw.on('drawend', (e) => {
    ...
    if (mode === 'CompoundCurve' || mode === 'CurvePolygon') {
      // Sketch is already canonical (traced runs converted at traceend), so no
      // post-hoc remapping is needed.
      ...
      // Tripwire: surface any tessellated point that leaked into a canonical
      // trace run instead of silently committing bad geometry.
