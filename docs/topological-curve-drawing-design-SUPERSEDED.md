...
> ## ⚠️ SUPERSEDED — historical design, not the shipped implementation
>
> This document is an **early design proposal that was not implemented as written.**
> The implementation pivoted to a **`TraceSource` graph model**. For the actual,
> as-built design see **[`docs/topological-curve-drawing-report.md`](topological-curve-drawing-report.md)**.
>
> **What is stale here (do not treat as API):**
> - The split/slice primitives below — `CircularArc.splitAtAngle()`,
>   `CircularString.splitAtCoordinate()`, `CompoundCurve.slice()` and
>   `CompoundCurve.locateCoordinate()` — **were never built.** Tracing instead
>   reuses stored arc sub-geometries via the trace graph; there is no
>   coordinate/angle splitting of source geometries.
> - The claim that "OL caches tessellation results" is **false** —
>   `tessellate()` recomputes on every call. Coordinate matching relies on the
>   tessellation being *deterministic* for a given geometry/revision, not cached.
>
> **Recorded UX decision (documented nowhere else): vertex-only exit.**
> The trace graph lets a user enter and leave an existing boundary **only at arc
> endpoints / control-point granularity — never mid-arc.** This is the single
> biggest intentional behavioural divergence from the design below. It is a
> deliberate constraint: junctions between shared boundaries must be explicit
> control points in the underlying geometry data model, so a trace may only pivot
> where such a vertex already exists. Mid-arc entry/exit would require splitting a
> source arc (the phantom primitives above), which was rejected.
...
The coordinate matching works because `tessellate()` produces deterministic coordinates for a given geometry/revision (it recomputes on every call — results are **not** cached), and the trace system copies these exact values.
...
**Mitigation:** Both come from the same `tessellate()` call on the same geometry object with the same revision, which is deterministic (OL does **not** cache tessellation — it recomputes every call). Use epsilon-based comparison (1e-6) for safety.
