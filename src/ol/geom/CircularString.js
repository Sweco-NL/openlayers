...
import {VERTEX_MATCH_DISTANCE} from './flat/tolerances.js';
 ...
 *
 * The result is clamped to the inclusive range `[4, 512]`. The lower bound
 * guarantees a minimally curved appearance; the upper bound of 512 segments is
 * a hard cap that bounds work per arc. When a very small `tolerance` (relative
 * to `radius`) would demand more than 512 segments, the tolerance is
 * best-effort only and the realised chord-to-arc error may exceed it.
...
class CircularString extends SimpleGeometry {
  ...
  /**
   * Set the flat coordinates directly. Overridden so the arc-derived caches
   * (center-of-circle and drawable coordinates) are refreshed whenever the
   * flat coordinates are replaced.
   * @param {import("./Geometry.js").GeometryLayout} layout Layout.
   * @param {Array<number>} flatCoordinates Flat coordinates.
   * @override
   */
  setFlatCoordinates(layout, flatCoordinates) {
    super.setFlatCoordinates(layout, flatCoordinates);
    this.update();
  }
  ...
  closestPointXY(x, y, closestPoint, minSquaredDistance) {
    ...
    for (let i = 0, n = this.arcCount(); i < n; ++i) {
      ...
      const center = getCircleCenter(bx, by, mx, my, ex, ey);
      if (!center) {
        // degenerate (collinear) arc - treat it as the straight chord segment
        // from begin to end so hit-testing still sees the straight portion.
        const onChord = closestOnSegment(
          [x, y],
          [
            [bx, by],
            [ex, ey],
        ...
        const squaredDist =
          (x - onChord[0]) * (x - onChord[0]) +
          (y - onChord[1]) * (y - onChord[1]);
        if (squaredDist < minSquaredDistance) {
          minSquaredDistance = squaredDist;
          closestPoint[0] = onChord[0];
          closestPoint[1] = onChord[1];
        }
        continue;
      }
      const cx = center[0];
  ...
  closestPointOnArc(arcIndex, x, y) {
    ...
    const center = getCircleCenter(bx, by, mx, my, ex, ey);
    if (!center) {
      // degenerate (collinear) arc - project onto the straight chord
  ...
  getCoordinateAt(fraction, dest) {
    ...
    if (n === 0) {
      // match the base-class contract: return a coordinate (filled with NaN
      // for an empty geometry), never null
  ...
  setCoordinates(coordinates, layout) {
    assert(
      !coordinates ||
        ...
        (coordinates.length >= 3 && coordinates.length % 2 === 1),
  ...
  intersectsExtent(extent) {
    ...
    for (let i = 0, n = this.arcCount(); i < n; ++i) {
      ...
      if (!center) {
        // degenerate (collinear) arc - test the straight chord against the extent
        if (intersectsSegment(extent, [abx, aby], [aex, aey])) {
          return true;
        }
        continue;
      }
    ...
    // Proximity fallback: find the point of the curve closest to the extent's
    // center and test whether it falls inside the extent. This catches an arc
    // that grazes the extent when no control point is inside it and no arc/edge
    // crossing was detected above. NOTE: this is a closest-point-to-center
    // proximity test, NOT an enclosure test — it does not (and cannot) report an
    // extent that is fully enclosed by a large closed curve, because in that
    // case the nearest curve point lies far out on the ring, not inside the
    // extent.
   ...
   * sweep angle.
   *
   * The subdivision of each arc is capped at 512 segments (and floored at 4).
   * For a very small `tolerance` on a large arc this cap can be reached, in
   * which case `tolerance` is honored on a best-effort basis only and the
   * realised deviation may exceed the requested value.
   *
  ...
  findArcEndpointIndex(coord, lastMatch) {
    ...
    const cx = coord[0];
    const cy = coord[1];
    const TOLERANCE = VERTEX_MATCH_DISTANCE;
  ...
  subArc(startIdx, endIdx) {
    ...
    if (startIdx <= endIdx) {
      ...
      const isClosed =
        n >= 3 &&
        Math.abs(coords[0][0] - coords[n - 1][0]) < VERTEX_MATCH_DISTANCE &&
        Math.abs(coords[0][1] - coords[n - 1][1]) < VERTEX_MATCH_DISTANCE;
