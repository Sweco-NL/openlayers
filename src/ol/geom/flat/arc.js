 ...
 * @param {number} [tolerance] Angular tolerance in radians (defaults to {@link ANGULAR_EPSILON}).
...
export function getCircleCenter(bx, by, mx, my, ex, ey) {
  ...
  const dx = bx - ex;
  const dy = by - ey;
  if (dx * dx + dy * dy < COINCIDENT_POINT_SQ) {
  ...
  if (len1Sq < COINCIDENT_SEGMENT_SQ || len2Sq < COINCIDENT_SEGMENT_SQ) {
    return null;
  }
  ...
  // Scale-invariant collinearity test: |denom| = |d1||d2||sin(theta)|, so
  // dividing by the segment lengths yields sin(theta) directly. Comparing the
  // normalized value (rather than the raw area `denom`) keeps the threshold
  // meaningful at any coordinate magnitude.
  const sinSq = (denom * denom) / (len1Sq * len2Sq);
  if (sinSq < COLLINEAR_SIN * COLLINEAR_SIN) {
...
export function isFullCircle(bx, by, ex, ey) {
  const dx = bx - ex;
  const dy = by - ey;
  return dx * dx + dy * dy < COINCIDENT_POINT_SQ;
}
...
export function getArcBoundingCoords(bx, by, mx, my, ex, ey, cx, cy) {
  ...
  for (let i = 0; i < 4; ++i) {
    ...
    if (
      startToExtreme > ARC_EXTREME_ANGULAR_EPSILON &&
      startToExtreme < startToEnd - ARC_EXTREME_ANGULAR_EPSILON
    ) {
