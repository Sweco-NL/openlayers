...
export function getLineLineCrossingPoint(x1, y1, x2, y2, x3, y3, x4, y4) {
  ...
  if (Math.abs(denom) < PARALLEL_DETERMINANT) {
    return null;
  }
  ...
  if (
    t > PARAMETRIC_EPSILON &&
    t < 1 - PARAMETRIC_EPSILON &&
    u > PARAMETRIC_EPSILON &&
    u < 1 - PARAMETRIC_EPSILON
  ) {
...
export function getLineArcCrossingPoint(
  ...
  const angularTolerance = Math.min(
    MAX_ARC_ANGULAR_TOLERANCE,
    arcSweep * ARC_SWEEP_TOLERANCE_FRACTION,
  );

  const results = [];
  for (const t of tValues) {
    if (t <= PARAMETRIC_EPSILON || t >= 1 - PARAMETRIC_EPSILON) {
      continue;
    }
    const px = x1 + t * dx;
    const py = y1 + t * dy;
    ...
    for (let li = 0; li < 4; li += 2) {
      for (let ai = 0; ai < 4; ai += 2) {
        ...
        if (sdx * sdx + sdy * sdy < epsilonSq) {
          ...
          if (li === 0 && t < ENDPOINT_PARAMETRIC_FRACTION) {
            nearShared = true;
            break;
          }
          if (li === 2 && t > 1 - ENDPOINT_PARAMETRIC_FRACTION) {
            nearShared = true;
            break;
          }
        }
 ...
 * Same parameters as the two-arc crossing test.
 *
 * This is the canonical arc-arc crossing routine; the former single-crossing
 * variant was removed. Callers that only need the first crossing can read
 * `result[0]`.
...
export function getArcArcCrossingPoints(
  ...
  const isLine1 =
    !c1 || getArcRadius(c1[0], c1[1], b1x, b1y) > DEGENERATE_ARC_RADIUS;
  const isLine2 =
    !c2 || getArcRadius(c2[0], c2[1], b2x, b2y) > DEGENERATE_ARC_RADIUS;
  ...
  if (d > r1 + r2 + DISTANCE_EPSILON) {
    return [];
  }
  if (d < Math.abs(r1 - r2) - DISTANCE_EPSILON) {
    return [];
  }
  if (d < DISTANCE_EPSILON) {
    if (Math.abs(r1 - r2) < RADIUS_MATCH_EPSILON) {
    ...
    for (let ei = 0; ei < 4; ei += 2) {
      for (let ej = 0; ej < 4; ej += 2) {
        ...
        if (sdx * sdx + sdy * sdy < epsilonSq) {
          ...
          const threshold =
            Math.min(chord1Sq, chord2Sq) * ENDPOINT_PARAMETRIC_FRACTION_SQ;
          if (distSq < threshold) {
            nearSharedEndpoint = true;
            break;
          }
...
export function arcsAreEqual(a, b, toleranceSq) {
  if (toleranceSq === undefined) {
    toleranceSq = DEFAULT_SAME_ARC_TOLERANCE_SQ;
  }
...
/**
 * Find all self-intersection points within a single array of arcs. Tests every
 * non-identical arc pair (`i < j`) for crossings. Adjacent arcs (including a
 * ring-closure pair) share an endpoint by construction; they are not skipped
 * here because `getArcArcCrossingPoints` filters near-shared-endpoint
 * candidates, so any remaining crossing is a genuine "long way around"
 * intersection (e.g. arcs sweeping more than 180°).
 *
 * Arc format: [beginX, beginY, midX, midY, endX, endY]
 *
 * @param {Array<Array<number>>} arcs Array of arcs.
 * @param {number} epsilonSq Squared distance threshold for crossing detection.
 * @param {number} [sameArcToleranceSq] Tolerance for arc equality check.
 *   Default 1e-4.
 * @return {Array<Array<number>>} Array of [x, y] crossing points.
 */
export function getArcArraySelfIntersections(
  arcs,
  epsilonSq,
  sameArcToleranceSq,
) {
  ...
  for (let i = 0, ii = arcs.length; i < ii; ++i) {
    ...
    for (let j = i + 1; j < ii; ++j) {
      const b = arcs[j];
      if (arcsAreEqual(a, b, sameArcToleranceSq)) {
        continue;
      }
      const pts = getArcArcCrossingPoints(
        a[0],
