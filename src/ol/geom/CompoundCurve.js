...
import CircularString from './CircularString.js';
...
/**
 * Assert that a sub-geometry starts exactly where the previous one ends.
 * `getFlatCoordinates`/`tessellate` deduplicate the shared junction by
 * dropping the first coordinate of each subsequent sub-geometry; if the
 * sub-geometries are disjoint that dedup silently swallows a coordinate and
 * renders a phantom connection, so contiguity is a hard precondition.
 * @param {CurveSegment} previous Previous sub-geometry.
 * @param {CurveSegment} next Next sub-geometry.
 */
function assertContiguous(previous, next) {
  const end = previous.getLastCoordinate();
  const start = next.getFirstCoordinate();
  assert(
    end[0] === start[0] && end[1] === start[1],
    `CompoundCurve sub-geometries must be contiguous: expected next sub-geometry to start at [${end[0]}, ${end[1]}], got [${start[0]}, ${start[1]}]`,
  );
}
...
class CompoundCurve extends Geometry {
  ...
  init_(layout) {
    ...
    for (let i = 0, ii = this.geometries_.length; i < ii; ++i) {
      ...
      if (i > 0) {
        ...
        assertContiguous(this.geometries_[i - 1], this.geometries_[i]);
      }
    }
  }
  ...
  appendGeometry(geometry) {
    ...
    if (this.geometries_.length > 0) {
      ...
      assertContiguous(this.geometries_[this.geometries_.length - 1], geometry);
    }
  ...
  forEachCurveSegment(callback) {
    ...
    for (let i = 0, ii = geometries.length; i < ii; ++i) {
      ...
      if (geom.getType() === 'CircularString') {
        ...
        // LineString: emit degenerate arcs (midpoint = segment center)
...
/**
 * Create a curve geometry from an array of coordinates and a segment type.
 *
 ...
 */
export function coordinatesToCurveGeometry(coordinates, type) {
  if (type === undefined) {
    type = 'arc';
  }
  ...
  return new CompoundCurve([
    new CircularString(arcCoords),
    new LineString(tailCoords),
  ]);
}
