 ...
 * (static membership) or a Collection. When a Collection is supplied, features
 * added or removed later are tracked automatically; in both cases geometry
 * changes to member features invalidate the cached graph.
 ...
 * participate. Builds the graph lazily and exposes query methods used by
 * {@link module:ol/interaction/Draw~Draw} for vertex-only-exit tracing. The graph is
 * cached and rebuilt automatically when member geometries change or the feature
 * collection is mutated.
 */
class TraceSource extends Disposable {
  /**
  ...
  constructor(options) {
    ...
    /**
     * Per-feature `change` listener keys, indexed by feature uid. A feature's
     * `change` event fires both when its geometry mutates in place and when
     * `setGeometry` replaces the geometry (the feature re-wires its own
     * geometry listener), so listening at the feature level covers every
     * geometry-change case without tracking geometry instances directly.
     * @private
    ...
    /**
     * Collection add/remove listener keys (empty for the plain-Array path).
     * @private
     * @type {Array<import("../events.js").EventsKey>}
     */
    this.collectionKeys_ = [];

    this.registerFeatures_();
  }

  /**
   * Attach change listeners to the current features and, for a Collection,
   * add/remove listeners that keep the per-feature listeners in sync.
   * @private
   */
  registerFeatures_() {
    const features = this.features_;
    if (features instanceof Collection) {
      features.forEach((feature) => this.listenFeature_(feature));
      this.collectionKeys_.push(
        listen(features, CollectionEventType.ADD, this.handleFeatureAdd_, this),
        listen(
          features,
      ...
      for (let i = 0, ii = features.length; i < ii; ++i) {
        this.listenFeature_(features[i]);
      }
    }
  }
  ...
  /**
   * @private
   * @param {import("../Feature.js").default} feature Feature to listen to.
   */
  listenFeature_(feature) {
    const uid = getUid(feature);
    if (this.featureChangeKeys_[uid]) {
      return;
    }
    this.featureChangeKeys_[uid] = listen(
      feature,
      EventType.CHANGE,
  ...
  /**
   * @private
   * @param {import("../Feature.js").default} feature Feature to stop listening to.
   */
  unlistenFeature_(feature) {
    const uid = getUid(feature);
    const key = this.featureChangeKeys_[uid];
    if (key) {
      unlistenByKey(key);
      delete this.featureChangeKeys_[uid];
    }
  }
  ...
  /**
   * @private
   * @param {import("../Collection.js").CollectionEvent<import("../Feature.js").default>} event Collection event.
   */
  handleFeatureAdd_(event) {
    this.listenFeature_(event.element);
    this.invalidate();
  }
   ...
   * @param {import("../Collection.js").CollectionEvent<import("../Feature.js").default>} event Collection event.
   */
  handleFeatureRemove_(event) {
    this.unlistenFeature_(event.element);
    this.invalidate();
  }
  ...
  collectFromGeometry_(geometry, feature) {
    ...
    // Points, MultiGeometries, GeometryCollections and Circles carry no
    // traceable ring edges and are ignored.
  }
   ...
   * @return {TraceEdge|null} The new active edge, or null when the source has
   *     no edges, or (for the initial acquisition, `previous === null`) when
   *     the closest edge lies farther than `tolerance` from the cursor.
   */
  getActiveEdge(coordinate, tolerance, previous) {
    ...
    if (!previous) {
      // Initial acquisition: only latch onto an edge within tolerance so a
      // click far from the graph does not silently snap to a distant feature.
      return bestDist2 <= tol2 ? bestEdge : null;
    }
    // Fallback: if no neighbor candidate was found (should be impossible when
    // `previous` is a valid edge, since `previous` itself is always considered)
    // keep the previous edge so the cursor stays on the graph.
  ...
  /**
   * Detach all internal listeners. The instance must not be used after
   * disposal (call the public {@link module:ol/Disposable~Disposable#dispose}
   * method, which routes here exactly once).
   * @override
   */
  disposeInternal() {
    this.collectionKeys_.forEach(unlistenByKey);
    this.collectionKeys_.length = 0;
    for (const uid in this.featureChangeKeys_) {
      unlistenByKey(this.featureChangeKeys_[uid]);
    }
    this.featureChangeKeys_ = {};
