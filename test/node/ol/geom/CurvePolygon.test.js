...
import {linearRingsAreOriented} from '../../../../src/ol/geom/flat/orient.js';
...
describe('ol/geom/CurvePolygon.js', function () {
  ...
  describe('constructor input ring mutation', function () {
    it('does not reverse ring instances passed to the constructor', function () {
      // Outer given counter-clockwise, inner given clockwise: the eager
      // normalization used to reverse both rings in place, clobbering the
      // caller's instances. Construction must leave the inputs untouched.
      const outer = new LineString([
        [0, 0],
        [20, 0],
        [20, 20],
      ...
      const outerBefore = outer.getCoordinates().map((c) => c.slice());
      const innerBefore = inner.getCoordinates().map((c) => c.slice());
      new CurvePolygon([outer, inner]);
      expect(outer.getCoordinates()).to.eql(outerBefore);
      expect(inner.getCoordinates()).to.eql(innerBefore);
    });

    it('still produces correctly oriented flat coordinates', function () {
      // Outer given counter-clockwise, inner given clockwise: the raw rings are
      // left untouched, but the derived flat coordinates are normalized.
      const outer = new LineString([
        [0, 0],
        [0, 20],
        [20, 20],
      ...
      const flat = cp.getOrientedFlatCoordinates();
      const ends = cp.getTessellatedEnds();
      expect(linearRingsAreOriented(flat, 0, ends, 2)).to.be(true);
    });
  });
  ...
  describe('getFirstCoordinate / getLastCoordinate', function () {
    ...
    it('returns empty coordinates for empty geometry (never null)', function () {
      const cp = new CurvePolygon([]);
      expect(cp.getFirstCoordinate()).to.eql([]);
      expect(cp.getLastCoordinate()).to.eql([]);
    });
  });
  ...
  describe('construct empty', function () {
    ...
    it('returns non-null first/last coordinates (matching base contract)', function () {
      expect(cp.getFirstCoordinate()).to.eql([]);
      expect(cp.getLastCoordinate()).to.eql([]);
    });
  });
