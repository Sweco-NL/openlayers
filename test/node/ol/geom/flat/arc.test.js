...
describe('ol/geom/flat/arc.js', () => {
  ...
  describe('getCircleCenter', () => {
    ...
    // Pins the scale-invariant collinearity test (Phase 7). The collinearity
    // check normalizes the perpendicular-bisector determinant by the segment
    // lengths, so the null/non-null decision depends only on the arc's *shape*,
    // never on its coordinate magnitude.
    describe('scale invariance', () => {
      it('rejects a near-collinear arc at large scale (bogus-center guard)', () => {
        // sin(theta) ~= 2*(5e-6)/1e6 = 1e-11, below COLLINEAR_SIN (1e-10), so
        // these three points are collinear. The previous raw-area test compared
        // |denom| ~= 10 against 1e-10 and wrongly returned a huge bogus center.
        const center = getCircleCenter(0, 0, 1e6, 5e-6, 2e6, 0);
        expect(center).to.be(null);
      });

      it('resolves a genuine arc identically at unit and large scale', () => {
        const unit = getCircleCenter(0, 0, 1, 1, 2, 0);
        const scaled = getCircleCenter(0, 0, 1e6, 1e6, 2e6, 0);
        expect(unit).not.to.be(null);
        expect(scaled).not.to.be(null);
        // Center scales linearly with the coordinates: (1,0) -> (1e6, 0).
        expect(scaled[0] / 1e6).to.roughlyEqual(unit[0], 1e-6);
        expect(scaled[1] / 1e6).to.roughlyEqual(unit[1], 1e-6);
      });

      it('makes the collinearity decision independent of scale', () => {
        // A shape whose sin(theta) ~= 1e-11 is collinear at every scale.
        expect(getCircleCenter(0, 0, 1, 5e-12, 2, 0)).to.be(null);
        expect(getCircleCenter(0, 0, 1e6, 5e-6, 2e6, 0)).to.be(null);
        // A shape whose sin(theta) ~= 1e-8 is a (very flat) arc at every scale.
        expect(getCircleCenter(0, 0, 1, 5e-9, 2, 0)).not.to.be(null);
        expect(getCircleCenter(0, 0, 1e6, 5e-3, 2e6, 0)).not.to.be(null);
      });
    });
  });
