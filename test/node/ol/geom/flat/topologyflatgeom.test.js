...
describe('ol/geom/flat/topology.js', function () {
  ...
  describe('getArcArraySelfIntersections', function () {
    const eps = 1e-4;

    it('returns empty array for fewer than two arcs', function () {
      expect(getArcArraySelfIntersections([], eps)).to.eql([]);
      expect(getArcArraySelfIntersections([[1, 0, 0, 1, -1, 0]], eps)).to.eql(
        [],
      );
    });

    it('does not report adjacent arcs that only share an endpoint', function () {
      // Two upper semicircles joined at (-1, 0): (1,0)->(0,1)->(-1,0) then
      // (-1,0)->(-2,1)->(-3,0). They meet only at the shared endpoint, which
      // getArcArcCrossingPoints filters out, so there is no self-intersection.
      const arcs = [
        [1, 0, 0, 1, -1, 0],
        [-1, 0, -2, 1, -3, 0],
      ];
      expect(getArcArraySelfIntersections(arcs, eps)).to.eql([]);
    });

    it('reports a genuine crossing between two non-adjacent arcs', function () {
      // Two overlapping unit circles' upper semicircles centered at origin and
      // at (0.5, 0). They cross at a real interior point.
      const arcs = [
        [1, 0, 0, 1, -1, 0],
        [1.5, 0, 0.5, 1, -0.5, 0],
      ];
      const result = getArcArraySelfIntersections(arcs, eps);
      expect(result.length).to.be.greaterThan(0);
      for (const pt of result) {
        expect(pt.length).to.be(2);
        const d1 = Math.sqrt(pt[0] * pt[0] + pt[1] * pt[1]);
    ...
    it('skips identical arcs via the same-arc tolerance', function () {
      const arc = [1, 0, 0, 1, -1, 0];
      const arcs = [arc, arc.slice()];
      expect(getArcArraySelfIntersections(arcs, eps, 1e-4)).to.eql([]);
    });
  });
});
