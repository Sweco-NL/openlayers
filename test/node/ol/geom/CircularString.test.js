...
describe('ol/geom/CircularString.js', function () {
  ...
  describe('construct empty', function () {
    ...
    it('returns a non-null coordinate from getCoordinateAt', function () {
      const coord = cs.getCoordinateAt(0.5);
  ...
  describe('construct with 2D coordinates', function () {
    ...
    describe('#setFlatCoordinates', function () {
      it('refreshes derived caches after direct flat-coordinate mutation', function () {
        // populate the derived caches at the original location
        const before = cs.getDrawableFlatCoordinates();
        expect(before[0]).to.be(5);
        expect(before[1]).to.be(0);

        // move the whole arc +100 in x via the low-level flat setter
        cs.setFlatCoordinates('XY', [
          105,
          0,
          (Math.SQRT2 / 2) * 5 + 100,
          (Math.SQRT2 / 2) * 5,
        ...
        const drawable = cs.getDrawableFlatCoordinates();
        expect(drawable[0]).to.be(105);
        expect(drawable[1]).to.be(0);
        expect(drawable[6]).to.be(100);
        expect(drawable[7]).to.be(5);
        // center of circle should track the moved arc (~x=100)
        expect(drawable[4]).to.roughlyEqual(100, 0.01);
        expect(drawable[5]).to.roughlyEqual(0, 0.01);
      });
    });
  });
  ...
  describe('arcCount edge cases', function () {
    ...
    it('throws for a single point (odd count below 3)', function () {
      expect(function () {
  ...
  describe('#intersectsExtent()', function () {
    ...
    it('does not report a small extent enclosed by a large arc as intersecting', function () {
      // Pins the proximity-fallback semantics: intersectsExtent tests whether
      // the curve comes near/into the extent, NOT whether the extent is
      // enclosed by a closed curve. A tiny extent at the center of a large arc
      // touches no part of the curve and crosses no edge, so it must be false.
      const cs = new CircularString([
        [100, 0],
        [0, 100],
        [-100, 0],
      ]);
      expect(cs.intersectsExtent([-1, -1, 1, 1])).to.be(false);
    });
  });
  ...
  describe('degenerate arcs', function () {
    ...
    it('closestPointXY projects onto the straight chord of a collinear arc', function () {
      const cs = new CircularString([
        [0, 0],
        [5, 0],
        [10, 0],
      ...
      const closest = [0, 0];
      const dist = cs.closestPointXY(5, 5, closest, Infinity);
      // Closest point is the foot of the perpendicular on the chord, not an endpoint.
      expect(dist).to.roughlyEqual(25, 1e-9);
      expect(closest[0]).to.roughlyEqual(5, 1e-9);
      expect(closest[1]).to.roughlyEqual(0, 1e-9);
    });

    it('closestPointOnArc projects onto the chord of a collinear arc', function () {
      const cs = new CircularString([
        [0, 0],
        [5, 0],
        [10, 0],
      ]);
      const pt = cs.closestPointOnArc(0, 3, 4);
      // Perpendicular foot on the chord, not the nearest endpoint.
      expect(pt[0]).to.roughlyEqual(3, 1e-9);
      expect(pt[1]).to.roughlyEqual(0, 1e-9);
    });
    ...
    it('intersectsExtent detects a collinear chord crossing an extent with no control point inside', function () {
      const cs = new CircularString([
        [0, 0],
        [5, 0],
        [10, 0],
      ]);
      // Extent [2,-1,3,1] contains none of the control points (0,0)/(5,0)/(10,0)
      // but the straight chord passes through it at y=0.
      expect(cs.intersectsExtent([2, -1, 3, 1])).to.be(true);
    });
