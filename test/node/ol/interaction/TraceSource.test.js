...
describe('ol/interaction/TraceSource.js', function () {
  ...
  describe('query API', function () {
    ...
    it('getActiveEdge returns null for a far cursor with no previous edge', function () {
      const f = new Feature(
        new LineString([
          [0, 0],
          [10, 0],
      ...
      const ts = new TraceSource({features: [f]});
      // cursor is ~90 units from the nearest edge, well outside tolerance 5
      expect(ts.getActiveEdge([100, 100], 5, null)).to.be(null);
    });
  ...
  describe('live updates', function () {
    ...
    it('invalidates when a feature geometry is modified in place', function () {
      const geom = new LineString([
        [0, 0],
        [1, 1],
      ]);
      const ts = new TraceSource({features: [new Feature(geom)]});
      expect(ts.getEdgeCount()).to.be(1);
      geom.setCoordinates([
        [0, 0],
        [1, 1],
    ...
    it('invalidates when a feature geometry is replaced via setGeometry', function () {
      const f = new Feature(
        new LineString([
          [0, 0],
          [1, 1],
      ...
      expect(ts.getEdgeCount()).to.be(2);
      // mutating the replacement geometry also invalidates (listener re-wired)
      ...
      expect(ts.getEdgeCount()).to.be(3);
    });

    it('detaches per-feature listeners on remove and dispose (no leak)', function () {
      const f = new Feature(
        new LineString([
          [0, 0],
          [1, 1],
      ...
      const collection = new Collection([f]);
      const ts = new TraceSource({features: collection});
      expect(Object.keys(ts.featureChangeKeys_).length).to.be(1);
      collection.remove(f);
      expect(Object.keys(ts.featureChangeKeys_).length).to.be(0);
      collection.push(f);
      collection.push(
        new Feature(
          new LineString([
      ...
      expect(Object.keys(ts.featureChangeKeys_).length).to.be(2);
      ts.dispose();
      expect(Object.keys(ts.featureChangeKeys_).length).to.be(0);
    });

    it('stops reacting to a removed feature geometry change', function () {
      const f = new Feature(
        new LineString([
          [0, 0],
          [1, 1],
      ...
      collection.remove(f);
      expect(ts.getEdgeCount()).to.be(0);
      // mutating the removed feature must not resurrect it in the graph
      f.getGeometry().setCoordinates([
        [0, 0],
        [1, 1],
        [2, 2],
